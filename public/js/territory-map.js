(() => {
    const hiddenByDefault = new Set(['inactive', 'decorative']);
    ...
    const bootTerritoryMap = (root) => {
        ...
        window.L.imageOverlay(root.dataset.mapImageUrl, bounds).addTo(map);
        ...
        const shouldDraw = (hex) => {
            ...
            return hex.is_visible && !hiddenByDefault.has(hex.tile_type);
        };
        ...
        const renderHex = (hex) => {
            ...
            if (!layer) {
                layer = window.L.polygon(asLeafletPoints(hex.polygon_coordinates), tileStyle(hex, mode, selectedHex?.id));
    ...
    document.addEventListener('DOMContentLoaded', () => bootTerritoryMap(document.getElementById('territory-map-app')));
})();
