<div class="mb-6 flex flex-wrap gap-2">
    ...
    @if (auth()->user()?->loadMissing('permissions')->hasPermission('developer'))
        <a href="{{ route('businesses.index') }}" class="rounded-full border px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] {{ $marketplaceSection === 'businesses' ? 'border-[#7ead59]/40 bg-[#7ead59]/15 text-[#d7edc7]' : 'border-white/10 bg-white/5 text-white/70' }}">Player Businesses</a>
    @else
