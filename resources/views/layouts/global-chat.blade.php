...
>
    <section
        x-show="!hidden"
        class="pointer-events-auto fixed bottom-5 right-5 flex h-[30rem] w-[min(92vw,21rem)] max-w-[21rem] flex-col overflow-hidden rounded-[1.35rem] border border-white/10 bg-[linear-gradient(180deg,rgba(8,14,10,0.72),rgba(6,10,8,0.68))] shadow-2xl shadow-black/45 backdrop-blur-xl"
    >
        <header
        ...
        function globalChatBox({ stateUrl, sendUrl, csrfToken, currentCharacter, currentFaction }) {
            return {
                ...
                applyUnread(channel, previousLastId, nextLastId) {
                    ...
                    if (this.hidden || this.activeTab !== channel) {
