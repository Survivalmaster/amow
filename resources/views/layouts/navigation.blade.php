<nav x-data="{ open: false }" class="border-b border-white/10 bg-black/25 backdrop-blur">
    ...
    <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div class="hidden items-center gap-3 md:flex">
            @if ($navCharacter)
                <div class="mr-2 flex items-center gap-3 rounded-[1.5rem] border border-white/10 bg-white/5 px-4 py-2">
                    <div>
                        <p class="font-['Teko'] text-2xl uppercase tracking-[0.08em] leading-none">{{ $navCharacter->name }}</p>
                        <p class="mt-1 text-[11px] uppercase tracking-[0.2em] text-white/45">{{ $navCharacter->rank->name }} • {{ ucfirst($navCharacter->role_type) }}</p>
                    </div>
                    <div class="flex items-center gap-2 rounded-full bg-[#7ead59]/15 px-3 py-2 text-[#c2a84f]">
