...
@php(
    $operationsNav = array_values(array_filter([
        ...
        $navUser->canAccessAdmin() ? ['label' => 'Admin', 'route' => 'admin.dashboard', 'match' => ['admin.dashboard', 'admin.users.*', 'admin.characters.*', 'admin.refunds.*', 'admin.nation-logs.*', 'admin.factions.*', 'admin.cities.*', 'admin.locations.*', 'admin.items.*', 'admin.licences.*', 'admin.permissions.*', 'admin.statistics.*', 'admin.jobs.*', 'admin.changelogs.*', 'admin.map-markers.*', 'admin.discord.*', 'admin.discord-management.*', 'admin.nation-requisitions.*', 'admin.stock-market.*', 'admin.server-tools.*'], 'icon' => 'fa-solid fa-shield-halved'] : null,
...
<nav x-data="{ marketplaceOpen: {{ request()->routeIs('bank.*', 'store.*', 'businesses.*') ? 'true' : 'false' }}, ucpOpen: {{ request()->routeIs('characters.show', 'inventory.*', 'home.*') ? 'true' : 'false' }}, nationOpen: {{ request()->routeIs('nation.*') ? 'true' : 'false' }}, adminOpen: {{ request()->routeIs('admin.*') ? 'true' : 'false' }}, toggleGameGroup(key) { const next = !this[key]; ['marketplaceOpen', 'ucpOpen', 'nationOpen', 'adminOpen'].forEach(group => this[group] = false); this[key] = next; } }" class="amow-navigation border-white/10">
    <div id="amow-navigation" class="amow-navigation-panel flex flex-col" tabindex="-1" aria-label="Game navigation">
        @include('layouts.partials.navigation-brand', ['navigationTitle' => 'AMOW', 'navigationRoute' => 'dashboard'])
        @if ($navCharacter)
            <div class="amow-navigation-character relative mb-6 shrink-0 overflow-hidden rounded-[1.25rem] bg-[linear-gradient(180deg,rgba(15,27,20,0.95),rgba(8,15,11,0.92))] p-3 pl-5 shadow-xl shadow-black/25">
...
<button
                                type="button"
                                @click="toggleGameGroup('marketplaceOpen')" :aria-expanded="marketplaceOpen" aria-controls="nav-marketplaceOpen"
...
<button
                        type="button"
                        @click="toggleGameGroup('ucpOpen')" :aria-expanded="ucpOpen" aria-controls="nav-ucpOpen"
...
<button
                        type="button"
                        @click="toggleGameGroup('nationOpen')" :aria-expanded="nationOpen" aria-controls="nav-nationOpen"
                        class="flex items-center gap-3 rounded-xl px-3 py-3 text-[14px] font-semibold transition text-white/82 hover:bg-white/[0.05]"
                    >
...
<button
                            type="button"
                            @click="toggleGameGroup('adminOpen')" :aria-expanded="adminOpen" aria-controls="nav-adminOpen"
                            class="flex items-center gap-3 rounded-xl px-3 py-3 text-[14px] font-semibold transition text-white/82 hover:bg-white/[0.05]"
                        >
...
<div class="amow-nav-submenu-content grid gap-1">
        ...
        <form method="POST" action="{{ route('logout') }}" class="amow-navigation-footer mt-6 shrink-0">
            @csrf
