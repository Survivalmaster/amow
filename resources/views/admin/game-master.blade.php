        ...
        <section class="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl shadow-black/30">
            ...
            <form method="POST" action="{{ route('admin.game-master.events.store') }}" class="grid gap-4 lg:grid-cols-2" x-data="{ xpBoost: false, creditBoost: false }">
                ...
                <div class="grid gap-3 md:grid-cols-2">
                    ...
                    <input class="rounded-2xl border border-white/10 bg-black/25 px-4 py-3 disabled:opacity-40" name="xp_multiplier" type="number" min="1" max="5" step="0.1" value="2" :disabled="!xpBoost">
                </div>
                <div class="grid gap-3 md:grid-cols-2">
                    <label class="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-sm text-white/70">
                    ...
                    <input class="rounded-2xl border border-white/10 bg-black/25 px-4 py-3 disabled:opacity-40" name="credit_multiplier" type="number" min="1" max="5" step="0.1" value="2" :disabled="!creditBoost">
                </div>
                ...
                <form method="POST" action="{{ route('admin.game-master.events.update', $event) }}" class="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl shadow-black/30" x-data="{ xpBoost: @js($event->xp_multiplier_enabled), creditBoost: @js($event->credit_multiplier_enabled) }">
                    ...
                    <div class="grid gap-4 lg:grid-cols-2">
                        ...
                        <div class="grid gap-3 md:grid-cols-2">
                            ...
                            <input class="rounded-2xl border border-white/10 bg-black/25 px-4 py-3 disabled:opacity-40" name="xp_multiplier" type="number" min="1" max="5" step="0.1" value="{{ $event->xp_multiplier ?? 2 }}" :disabled="!xpBoost">
                        </div>
                        <div class="grid gap-3 md:grid-cols-2">
                            <label class="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-sm text-white/70">
                            ...
                            <input class="rounded-2xl border border-white/10 bg-black/25 px-4 py-3 disabled:opacity-40" name="credit_multiplier" type="number" min="1" max="5" step="0.1" value="{{ $event->credit_multiplier ?? 2 }}" :disabled="!creditBoost">
                        </div>
