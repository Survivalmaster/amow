<x-app-layout>
    ...
    >
        ...
        <section x-show="showCreate" x-cloak class="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl shadow-black/30">
            ...
            <form method="POST" action="{{ route('admin.permissions.store') }}" class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                @csrf
                        ...
                        @foreach ($permissions as $permission)
                            ...
                            <tr x-show="openId === {{ $permission->id }}" x-cloak>
                                <td colspan="7" class="px-5 pb-5">
                                    <form method="POST" action="{{ route('admin.permissions.update', $permission) }}" class="grid gap-4 rounded-[1.5rem] border border-white/10 bg-black/20 p-5 md:grid-cols-2 xl:grid-cols-4">
                                        @csrf
                                        @method('PATCH')
                                        <label class="grid gap-2 text-sm text-white/70">
                                            <span class="uppercase tracking-[0.18em] text-white/45">Name</span>
